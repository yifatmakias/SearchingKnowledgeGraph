...
public class GreedyQuery {
    ...
    private Map<DefaultListenableGraph, Double> graphResultsMap;
    ...
    public GreedyQuery(RDFGraph graph, GraphQuery graphQuery, String simFileEdge, String simFileNode, String graphDomain) {
        ...
        this.graphResultsMap = new HashMap<>();
    }
                    ...
                    if (results[i][j] != null) {
                        for (int pathNode : results[i][j].getPath().getNodes()) {
                            if (topKMap.keySet().contains(graph.getNodeData(pathNode).getName())) {
                                ...
                                for (Map.Entry<DefaultListenableGraph, Double> entry : this.graphResultsMap.entrySet()) {
                                    if (entry.getKey().containsVertex(startNodeName)) {
                                        entry.getKey().addVertex(endNodeName);
                                        RelationshipEdge edge = new RelationshipEdge(predicates.get(0));
                                        edge.setSource(startNodeName);
                                        edge.setTarget(endNodeName);
                                        entry.getKey().addEdge(startNodeName, endNodeName, edge);
                                        entry.setValue((entry.getValue() + rank) / 2);
                                        newGraph = false;
                                    }
                                }
                                ...
                                if (newGraph) {
                                    DefaultListenableGraph graph =
                                            new DefaultListenableGraph<String, DefaultEdge>(new SimpleDirectedGraph<String, DefaultEdge>(DefaultEdge.class));
                                    graph.addVertex(startNodeName);
                                    graph.addVertex(endNodeName);
                                    RelationshipEdge edge = new RelationshipEdge(predicates.get(0));
                                    edge.setSource(startNodeName);
                                    edge.setTarget(endNodeName);
                                    graph.addEdge(startNodeName, endNodeName, edge);
                                    this.graphResultsMap.put(graph, rank);
                                }
                                if (subPath == null){
                                    subPath = new ArrayList<>();
    ...
    public Map<DefaultListenableGraph, Double> getGraphResultsMap() {
        return graphResultsMap;
    }

    public void printGraphResultMap() {
        for (Map.Entry<DefaultListenableGraph, Double> entry : this.graphResultsMap.entrySet()) {
